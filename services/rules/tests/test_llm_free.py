"""B11 LLM-free Path Integrity —— rules 那一段。
...
PACKAGE_ROOT = pathlib.Path(campuspath_rules.__file__).resolve().parent
DISTRIBUTION = "campuspath-rules"
...
def test_layer1_runtime_imports():
    for info in pkgutil.iter_modules(campuspath_rules.__path__):
        importlib.import_module(f"campuspath_rules.{info.name}")
