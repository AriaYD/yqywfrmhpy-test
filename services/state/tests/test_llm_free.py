"""B11 LLM-free Path Integrity —— state 那一段。
...
PACKAGE_ROOT = pathlib.Path(campuspath_state.__file__).resolve().parent
DISTRIBUTION = "campuspath-state"
...
def test_layer1_runtime_imports():
    for info in pkgutil.iter_modules(campuspath_state.__path__):
        importlib.import_module(f"campuspath_state.{info.name}")
