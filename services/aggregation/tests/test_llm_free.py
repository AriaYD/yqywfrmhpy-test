"""B11 LLM-free Path Integrity —— aggregation 那一段。
...
PACKAGE_ROOT = pathlib.Path(campuspath_aggregation.__file__).resolve().parent
DISTRIBUTION = "campuspath-aggregation"
...
def test_layer1_runtime_imports():
    for info in pkgutil.iter_modules(campuspath_aggregation.__path__):
        importlib.import_module(f"campuspath_aggregation.{info.name}")
