"""B11 LLM-free Path Integrity —— capacity 那一段。
...
PACKAGE_ROOT = pathlib.Path(campuspath_capacity.__file__).resolve().parent
DISTRIBUTION = "campuspath-capacity"
...
def test_layer1_runtime_imports():
    for info in pkgutil.iter_modules(campuspath_capacity.__path__):
        importlib.import_module(f"campuspath_capacity.{info.name}")
