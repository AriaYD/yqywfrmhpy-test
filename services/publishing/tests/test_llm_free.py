"""B11 LLM-free Path Integrity —— publishing 那一段。
...
PACKAGE_ROOT = pathlib.Path(campuspath_publishing.__file__).resolve().parent
DISTRIBUTION = "campuspath-publishing"
...
def test_layer1_runtime_imports():
    for info in pkgutil.iter_modules(campuspath_publishing.__path__):
        importlib.import_module(f"campuspath_publishing.{info.name}")
