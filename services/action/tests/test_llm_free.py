"""B11 LLM-free Path Integrity —— action 那一段。
...
PACKAGE_ROOT = pathlib.Path(campuspath_action.__file__).resolve().parent
DISTRIBUTION = "campuspath-action"
...
def test_layer1_runtime_imports():
    for info in pkgutil.iter_modules(campuspath_action.__path__):
        importlib.import_module(f"campuspath_action.{info.name}")
