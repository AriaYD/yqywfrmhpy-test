"""B11 LLM-free Path Integrity —— wellbeing 那一段。
...
PACKAGE_ROOT = pathlib.Path(campuspath_wellbeing.__file__).resolve().parent
DISTRIBUTION = "campuspath-wellbeing"
...
def test_layer1_runtime_imports():
    for info in pkgutil.iter_modules(campuspath_wellbeing.__path__):
        importlib.import_module(f"campuspath_wellbeing.{info.name}")
