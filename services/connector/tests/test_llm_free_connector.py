"""B11 LLM-free Path Integrity —— connector 那一段。
...
PACKAGE_ROOT = pathlib.Path(campuspath_connector.__file__).resolve().parent
DISTRIBUTION = "campuspath-connector"
...
def test_layer1_runtime_imports():
    for info in pkgutil.iter_modules(campuspath_connector.__path__):
        importlib.import_module(f"campuspath_connector.{info.name}")
