"""B11 LLM-free Path Integrity —— packs 那一段（vendored 国际生 Pack 求值器）。
...
PACKAGE_ROOT = pathlib.Path(campuspath_packs.__file__).resolve().parent
DISTRIBUTION = "campuspath-packs"
...
def test_layer1_runtime_imports():
    for info in pkgutil.iter_modules(campuspath_packs.__path__):
        importlib.import_module(f"campuspath_packs.{info.name}")
