"""B11 LLM-free Path Integrity —— mock-campus 那一段。
...
PACKAGE_ROOT = pathlib.Path(campuspath_mock_campus.__file__).resolve().parent
DISTRIBUTION = "campuspath-mock-campus"
...
def test_layer1_runtime_imports():
    for info in pkgutil.iter_modules(campuspath_mock_campus.__path__):
        importlib.import_module(f"campuspath_mock_campus.{info.name}")
